...
recall. If the 38 negative rows are loaded as positives instead, the same perfect model is capped at
87/125 ≈ 0.70, and every candidate is charged 0.30 recall for a loader artefact. That is the whole reason
the adapter exists, so it is measured here rather than argued.
...
def test_a_perfect_model_is_not_capped(adapted: dict[str, Any]) -> None:
    """Recall 65/65 = 1.00 through the adapter; 87/125 = 0.696 through the naive load."""
    adapted_recall = EXPECTED["claims"] / adapted["reconciliation"]["scored_recall_denominator"]
    naive_recall = (EXPECTED["rows"] - NEGATIVE_ROWS) / EXPECTED["rows"]
    assert adapted_recall == 1.0
    assert round(naive_recall, 3) == 0.696
    assert adapted_recall - naive_recall > 0.30
...
def test_the_naive_load_caps_the_same_model_at_070(
    ...
    """The counterfactual, measured: all 125 rows as positives, same perfect model, recall ≈ 0.70."""
    gold_mod, matcher_mod, surface_mod = scorer
