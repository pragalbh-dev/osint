...
| 6 | **−1 `supersedes` edge**; the retired basing goes `stale` → `possible` | The flagship relocation lands in the **third state**: one shared untagged instance (no de-confliction), nomination withdrawn (no fusion), and a named gap. Its two ends are described on different axes, so one class maps and one does not, and we cannot yet say whether this is one kind of place. **Intended and ruled correct** — the mapping is DATA's to supply. |
