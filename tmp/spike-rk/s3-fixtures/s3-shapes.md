...
| Fixture | Corpus |
|---|---|
| `F1a-explicit-equivalence-clean` | NOT EXERCISABLE as a coreference fixture. The frozen bundles carry no coreference annotation of any kind - the producer is switched off - so there is no cluster, no category and no licensing quote anywhere in the frozen data to test against, and the only route to one is a keyed re-extract. The raw PROSE shape does exist: the corpus contains a textbook long-name-plus-parenthetical apposition. That is a warning rather than a comfort, because that particular apposition is a deliberate demo beat an analyst is meant to earn, and switching the producer on would auto-bind it. |
| `F1b-quote-missing-one-surface-form` | NOT EXERCISABLE. Needs a cluster whose quote is genuine and insufficient; there are no clusters. Note also that this defect cannot be scheduled even with a re-extract - a quote that under-licenses is a model behaviour, observable but not commandable, which is exactly why it has to be authored. |
| `F1c-quote-is-an-enumeration-not-an-equivalence` | NOT EXERCISABLE as a bind. The raw material for the failure is present - the corpus does enumerate two designs in one breath in several places - but the mislabelled cluster over it does not exist and cannot be produced on demand. |
...
| `F2b-mark-vs-name-nothing-may-be-asserted` | EXERCISABLE IN KIND, and live in the frozen data: a base design mark and a more specific mark of which it is a prefix are the corpus's own central designator ambiguity, and the shipped containment knob states the doctrine in exactly these terms (an extra descriptive WORD describes the same thing more fully; an extra MARK is a different thing). The corpus even has the harder version - one source blurs the two marks while the answer key grades them a confirmed distinction on a hedged weaker source. What is NOT exercisable is the labelled bind: the pair never arrives as a cluster, so the gate is never offered the case. |
| `F2c-mark-parenthetical-passes-the-marker-gate` | NOT EXERCISABLE, and the highest-value authored case in this shape. The corpus contains the mark pair and contains parenthetical appositions, but never the two combined in one span - so the one input that makes the deterministic marker test pass on a wrong bind does not exist anywhere in the frozen data, and cannot be conjured without a re-extract that might or might not produce it. |
...
| `F3a-anaphor-positive-gate-passes` | NOT EXERCISABLE. Anaphoric prose is abundant in the corpus - documents refer back to the battery, the system, the site - but the gate turns on the document's MENTION INVENTORY, which only exists once the producer runs. Nothing in the frozen data exposes one. |
| `F3b-anaphor-two-candidates-of-the-same-kind` | NOT EXERCISABLE at body level, and this is a structural limit rather than an accident: across the 52 scenario documents there is essentially one numbered body (an off-subject decoy), so two candidate bodies of one kind cannot occur in one document. The same ambiguity at design or site level is plausible in the corpus prose but was not verified per-document in this pass. |
...
| `F3d-anaphor-with-no-named-antecedent-at-all` | PARTIALLY EXERCISABLE in substance. The corpus genuinely contains bodies referred to only by descriptor, and - better - it contains stated absences of identity, including one document stating the absence across its whole pass history and one stating a non-disclosure policy, so the 'this will not close' reason is real data. The single-member cluster that carries it is not. |
...
| `F6a-shared-designation-different-operators` | PARTIALLY EXERCISABLE. At DESIGN level the corpus genuinely carries one designation associated with two different operators in two countries, which is this fixture's shape one layer down. At BODY level it is not exercisable: one numbered body in 52 documents, and it is a planted designator-string collision belonging to a foreign army. |
...
| `F6d-title-reissued-after-a-stated-disestablishment` | NOT EXERCISABLE. No disestablishment or re-raising statement exists in the corpus, and there are no body titles to carry one. Note this rung of the ladder - continuity witnessed in time - has no corpus data anywhere, in either direction. |
| `F7a-operator-stated-on-both-sides-and-uncomparable` | EXERCISABLE, and already conceded in the shipped config: the body's branch discriminator is declared soft PRECISELY because the corpus states it unnormalised across three different forms, and the config comment says so in as many words. That is this fixture's problem sitting live in the frozen data with the workaround already applied - and the workaround is the wrong one of the three states (soft, i.e. fusable, rather than neither-wall-nor-fuse). What the corpus does not supply is a candidate merge whose two sides state two irreconcilable forms; in the corpus the forms mostly co-occur in the same documents. |
...
| `F7c-containment-is-not-agreement` | EXERCISABLE IN KIND. The corpus contains a joint-use compound naming two services in one string (verified in the S2 pass, as a stated site-kind value), so the containment trap has real material - a string that literally contains one operator's name while denoting a joint arrangement. |
...
| `F8c-coordinates-identify-a-place` | LIKELY EXERCISABLE, not re-verified in this pass. The corpus was deliberately seeded with coordinates in more than one notation and with per-node-type precision classes, and the shipped ontology already comments that a place's coordinates identify the place. I did not re-check the per-document notations, so treat the two-notations-one-point pair as authored. |
| `F8d-one-name-two-points-and-one-with-no-point` | NOT VERIFIED, and probably absent. Places whose coordinates are missing are common in the corpus, but a single place NAME attached to two materially different stated positions is not something I confirmed, and it would most likely be read as a data error rather than as the identity trap it is. |
| `F9a-enumeration-with-a-stated-count` | PARTIALLY EXERCISABLE. The sourced-figure half is real - the corpus states order-of-battle figures in prose - and the destination now exists, since the shipped ontology declares the count attribute as sourced-only and explicitly never derived from how many reports merged. What is absent is the enumeration of two INDIVIDUATED bodies in one document, so the contrast channel has no corpus input. |
...
| `F10a-same-title-two-operators-must-not-fuse` | EXERCISABLE IN KIND, and deliberately planted: the corpus carries one designator-string collision decoy - a foreign body whose title collides with subject-side strings. Being off-subject limits what it demonstrates, but it is a genuine cross-operator same-string pair and it is the one case in the corpus where this gate can bite. |
| `F10b-one-name-three-kinds-of-thing` | NOT VERIFIED in this pass as a three-way collision. The corpus does contain both area-class and site-class places with overlapping naming, and the ontology had to grow a refinement to keep areas out of the site lane, so the site-versus-area half of the fixture has real precedent. |
| `F10c-the-alias-route-must-be-gated-too` | NOT EXERCISABLE from corpus data, and the hazard was not found there in the first place - it came from the code and config review. The seed place gazetteer does carry real aliases (one deliberately withheld so a merge can be earned), so alias entries with content exist; I did not check whether any entry has the same string on both sides or bridges two kinds of thing. The self-referential entry in this fixture is authored to make the route testable. |
| `F11a-thin-mention-must-stay-under-determined` | PARTIALLY EXERCISABLE. Thin design-only mentions are real in the corpus, largely in chaff documents, and the corpus supplies something better than a thin mention - stated absences of identity, including a non-disclosure policy, which is a gap that will not close from official sources. What it does not supply is the trap at full strength: the corpus holds several presences of the subject design, so a thin mention arriving into a graph with exactly ONE candidate - the condition that makes the only-candidate fallacy irresistible - has to be authored. |
...
**Corpus:** NOT EXERCISABLE as a coreference fixture. The frozen bundles carry no coreference annotation of any kind - the producer is switched off - so there is no cluster, no category and no licensing quote anywhere in the frozen data to test against, and the only route to one is a keyed re-extract. The raw PROSE shape does exist: the corpus contains a textbook long-name-plus-parenthetical apposition. That is a warning rather than a comfort, because that particular apposition is a deliberate demo beat an analyst is meant to earn, and switching the producer on would auto-bind it.
...
**Corpus:** NOT EXERCISABLE. Needs a cluster whose quote is genuine and insufficient; there are no clusters. Note also that this defect cannot be scheduled even with a re-extract - a quote that under-licenses is a model behaviour, observable but not commandable, which is exactly why it has to be authored.
...
**Corpus:** NOT EXERCISABLE as a bind. The raw material for the failure is present - the corpus does enumerate two designs in one breath in several places - but the mislabelled cluster over it does not exist and cannot be produced on demand.
...
**Corpus:** EXERCISABLE IN KIND, and live in the frozen data: a base design mark and a more specific mark of which it is a prefix are the corpus's own central designator ambiguity, and the shipped containment knob states the doctrine in exactly these terms (an extra descriptive WORD describes the same thing more fully; an extra MARK is a different thing). The corpus even has the harder version - one source blurs the two marks while the answer key grades them a confirmed distinction on a hedged weaker source. What is NOT exercisable is the labelled bind: the pair never arrives as a cluster, so the gate is never offered the case.
...
**Corpus:** NOT EXERCISABLE, and the highest-value authored case in this shape. The corpus contains the mark pair and contains parenthetical appositions, but never the two combined in one span - so the one input that makes the deterministic marker test pass on a wrong bind does not exist anywhere in the frozen data, and cannot be conjured without a re-extract that might or might not produce it.
...
**Corpus:** NOT EXERCISABLE. Anaphoric prose is abundant in the corpus - documents refer back to the battery, the system, the site - but the gate turns on the document's MENTION INVENTORY, which only exists once the producer runs. Nothing in the frozen data exposes one.
...
**Corpus:** PARTIALLY EXERCISABLE in substance. The corpus genuinely contains bodies referred to only by descriptor, and - better - it contains stated absences of identity, including one document stating the absence across its whole pass history and one stating a non-disclosure policy, so the 'this will not close' reason is real data. The single-member cluster that carries it is not.
...
**Corpus:** PARTIALLY EXERCISABLE. At DESIGN level the corpus genuinely carries one designation associated with two different operators in two countries, which is this fixture's shape one layer down. At BODY level it is not exercisable: one numbered body in 52 documents, and it is a planted designator-string collision belonging to a foreign army.
...
**Corpus:** NOT EXERCISABLE. No disestablishment or re-raising statement exists in the corpus, and there are no body titles to carry one. Note this rung of the ladder - continuity witnessed in time - has no corpus data anywhere, in either direction.
...
**Corpus:** EXERCISABLE, and already conceded in the shipped config: the body's branch discriminator is declared soft PRECISELY because the corpus states it unnormalised across three different forms, and the config comment says so in as many words. That is this fixture's problem sitting live in the frozen data with the workaround already applied - and the workaround is the wrong one of the three states (soft, i.e. fusable, rather than neither-wall-nor-fuse). What the corpus does not supply is a candidate merge whose two sides state two irreconcilable forms; in the corpus the forms mostly co-occur in the same documents.
...
**Corpus:** EXERCISABLE IN KIND. The corpus contains a joint-use compound naming two services in one string (verified in the S2 pass, as a stated site-kind value), so the containment trap has real material - a string that literally contains one operator's name while denoting a joint arrangement.
...
**Corpus:** LIKELY EXERCISABLE, not re-verified in this pass. The corpus was deliberately seeded with coordinates in more than one notation and with per-node-type precision classes, and the shipped ontology already comments that a place's coordinates identify the place. I did not re-check the per-document notations, so treat the two-notations-one-point pair as authored.
...
**Corpus:** NOT VERIFIED, and probably absent. Places whose coordinates are missing are common in the corpus, but a single place NAME attached to two materially different stated positions is not something I confirmed, and it would most likely be read as a data error rather than as the identity trap it is.
...
**Corpus:** PARTIALLY EXERCISABLE. The sourced-figure half is real - the corpus states order-of-battle figures in prose - and the destination now exists, since the shipped ontology declares the count attribute as sourced-only and explicitly never derived from how many reports merged. What is absent is the enumeration of two INDIVIDUATED bodies in one document, so the contrast channel has no corpus input.
...
**Corpus:** EXERCISABLE IN KIND, and deliberately planted: the corpus carries one designator-string collision decoy - a foreign body whose title collides with subject-side strings. Being off-subject limits what it demonstrates, but it is a genuine cross-operator same-string pair and it is the one case in the corpus where this gate can bite.
...
**Corpus:** NOT VERIFIED in this pass as a three-way collision. The corpus does contain both area-class and site-class places with overlapping naming, and the ontology had to grow a refinement to keep areas out of the site lane, so the site-versus-area half of the fixture has real precedent.
...
**Corpus:** NOT EXERCISABLE from corpus data, and the hazard was not found there in the first place - it came from the code and config review. The seed place gazetteer does carry real aliases (one deliberately withheld so a merge can be earned), so alias entries with content exist; I did not check whether any entry has the same string on both sides or bridges two kinds of thing. The self-referential entry in this fixture is authored to make the route testable.
...
**Corpus:** PARTIALLY EXERCISABLE. Thin design-only mentions are real in the corpus, largely in chaff documents, and the corpus supplies something better than a thin mention - stated absences of identity, including a non-disclosure policy, which is a gap that will not close from official sources. What it does not supply is the trap at full strength: the corpus holds several presences of the subject design, so a thin mention arriving into a graph with exactly ONE candidate - the condition that makes the only-candidate fallacy irresistible - has to be authored.
