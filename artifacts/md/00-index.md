...
1. *(the take-home brief itself is held locally and is not tracked in this repo)*
...
`chat.txt` is untouched in the parent `artifacts/` folder. The three source `.pdf`s are held locally only and are not tracked in this repo.
