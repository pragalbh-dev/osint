...
| **`Entity.namespace()` reads `origin_country`, and the un-normalised branch folds case + punctuation** | `origin_country` is **the only country attribute the corpus states** (on manufacturers and trading organisations); `country` is stated nowhere. Measured: two same-named trading orgs, one CHINA and one Pakistan, fused at `confirmed` in **both** flag directions, while the identical pair keyed on `country` was correctly refused — and G19's fixtures all spelled it `country`, which is why the gate was green while the harm it names happened | Adding the key alone split the corpus's own SINO-GALAXY pair ('CHINA' vs 'China') into two nodes (169 → 170), because C7's value normaliser is flag-gated. So the raw branch folds through the same `fold_value` the normaliser uses: **case is never a namespace difference, with the flag or without it**. Folding is byte-inert on the corpus on its own. G19's Phase-2 refusal and its must-fuse control are parametrized over both keys |
