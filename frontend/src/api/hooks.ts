...
/** LIVE anchor check — which armed tripwires resolve to nothing (AH-1).
 *
 *  An observable anchored on an instance id that no longer resolves watches an EMPTY SET: it fires
 *  nothing, raises nothing, and logs nothing, so "no alerts" reads as an all-clear when it is really
 *  "not looking". The backend recomputes this on every catalogue read against the live view, and it
 *  rides in on the same request the armed count already makes — no extra fetch, no stale verdict.
 *
 *  Returns `null` when unknown (demo mode, in flight, fetch failed, or the backend could not run the
 *  check) so a caller can say "unknown" instead of rendering an unearned clean bill of health. */
export function useAnchorCheck(): AnchorCheck | null {
  const mode = useWorkbench((s) => s.mode)
  const { data } = useQuery({
    queryKey: ['config', 'observables'],
    queryFn: () => api.configSection('observables'),
