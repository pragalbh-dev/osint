...
export function Rail() {
  ...
  // Derive from the stable `decided` reference — never via an array-returning store
  // selector, which returns a fresh array each call and loops under useSyncExternalStore.
  const decided = useWorkbench((s) => s.decided)
  const reviewItems = QUEUE_ITEMS.filter((i) => !decided[i.id])
  ...
  return (
    <aside className="flex w-[240px] flex-none flex-col border-r border-hairline bg-surface">
            ...
            {reviewItems.map((item) => (
              ...
              >
                <div className="mb-1 flex items-center justify-between gap-2">
                  <span className="text-[10.5px] tracking-[0.04em] text-text-faint">{item.type}</span>
                  <span className="whitespace-nowrap rounded-[4px] border border-hairline-strong px-[7px] py-[1px] text-[10px] text-text-dim">
                    {item.badge}
                  </span>
                </div>
                <div className="text-[12.5px] leading-[1.35] text-text">{item.subject}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {/* Ingest — the keyless bundle / scripted-trace demo surface. The trace itself
          renders on the stage, not here. */}
